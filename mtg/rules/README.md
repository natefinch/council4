...
- Effect primitive execution for drawing cards, gaining life, losing life, player damage, permanent damage, destroy, counter, exile, bounce, sacrifice, discard, tap/untap, adding/removing/moving counters, mass permanent/player selector effects, token creation/investigate with referenced recipients, supported library-to-hand search, reveal, runtime P/T modifiers, declarative runtime continuous and rule effects, condition-gated effects, referenced damage sources for creature-sourced damage, event-derived/opponent-count dynamic amounts, scry/surveil/mill, fight, transform, phase out, emblems, proliferate, goad, prevention, regeneration, and delayed triggers. Unsupported effect primitives and unsupported variants are logged instead of silently no-oping.
...
- Triggered ability detection from `game.TriggerPattern`, choice-mediated target selection, APNAP stack placement with choice-mediated same-controller ordering, optional-trigger resolution choices, spell/permanent type filters, structured intervening-if conditions, supported beginning-of-step triggers, latched state triggers, implicit Prowess triggers, and resolution through `StackTriggeredAbility`.
