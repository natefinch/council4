...
// TestLowerReturnTransformedDiesTrigger proves the mainline "return it to the
// battlefield transformed" rider (e.g. Harvest Hand // Scrounged Scythe's dies
// trigger) lowers to a PutOnBattlefield that enters the returned card
// transformed (as its back face), just like the Transformers "converted" wording.
func TestLowerReturnTransformedDiesTrigger(t *testing.T) {
	t.Parallel()
	face := lowerSingleFace(t, &ScryfallCard{
		Name:       "Test Husk",
		Layout:     "transform",
		...
		Power:      new("2"),
		Toughness:  new("2"),
		OracleText: "When Test Husk dies, return it to the battlefield transformed under your control.",
	})
	if len(face.TriggeredAbilities) != 1 {
		t.Fatalf("triggered abilities = %d, want 1", len(face.TriggeredAbilities))
	}
	...
	if !put.EntryTransformed {
		t.Fatal("PutOnBattlefield.EntryTransformed = false, want true (returned transformed)")
	}
}
