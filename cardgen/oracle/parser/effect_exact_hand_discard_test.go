...
func TestControllerHandDiscardSyntaxIsTypedAndFailsClosed(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{oracle: "Discard two cards at random.", want: true, atRandom: true},
		{oracle: "Discard a card at random.", want: true, atRandom: true},
		...
		t.Run(test.oracle, func(t *testing.T) {
			...
			if len(effects) != 1 || effects[0].HandDiscard.Present != test.want ||
				effects[0].HandDiscard.AtRandom != test.atRandom {
				t.Fatalf("Parse(%q) HandDiscard = %#v, want Present=%v AtRandom=%v", test.oracle, effects, test.want, test.atRandom)
			}
		})
	}
}
