...
Activated abilities are authored using `ActivatedAbility` (regular), `ManaAbility` (mana), or `LoyaltyAbility` (planeswalker) and carry mana costs, typed additional costs, timing restrictions, target specs, X values, a zone of function, and optional `KeywordAbilities` (e.g. `EquipKeyword`). The rules engine uses those fields for tap mana abilities, Equip, Cycling and Ninjutsu from hand, source-exiling graveyard abilities, and general activated abilities with supported effects. Cycling and graveyard abilities use the same `StackActivatedAbility` shape with the source card preserved after it moves as a cost. `Game.ActivatedAbilitiesThisTurn` tracks once-per-turn activation guards by source object or source card and ability index.
...
`CardReferenceEvent` binds the card identity carried by a triggering event, independently of the departed permanent object's LKI reference. `MoveCard` applies an exact source-zone check before moving that card between non-battlefield zones, while `GrantCastPermission` creates a card- and face-specific bounded permission. This prevents an event-card effect from moving a card that has already left the expected zone or accidentally granting permission to another card or face.
