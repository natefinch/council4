...
Triggered abilities use `TriggerCondition.Pattern` to match typed `GameEvent` values. The first trigger slice supports exact event-kind matching plus filters for controller, source/self, affected player, permanent/card type include/exclude filters, zone transition, damage recipient, and beginning-of-step events for supported steps. Optional "you may" triggered abilities set `AbilityDef.Optional`; they still use the stack, and the rules engine asks for the yes/no choice when they resolve. The legacy `TriggerCondition.Event` string is documentation only and is not used for rules behavior.
