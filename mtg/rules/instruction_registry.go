...
func newPrimitiveRegistry() *primitiveRegistry {
	...
	registerPrimitiveHandler(reg, handleMoveResolvingSpell)
	...
	registerPrimitiveHandler(reg, handleReturnExiledCardsWithCounter)
	registerPrimitiveHandler(reg, handleChampionExile)
	registerPrimitiveHandler(reg, handleReturnLinkedExiledCardsToBattlefield)
	registerPrimitiveHandler(reg, handleChooseCardFromEachGraveyard)
	registerPrimitiveHandler(reg, handleReanimateLinkedCards)
	registerPrimitiveHandler(reg, handleEachPlayerChooseDestroy)
	registerPrimitiveHandler(reg, handleOptionalCounterForEachPlayer)
	registerPrimitiveHandler(reg, handleCreateTokenForEachDestroyed)
	registerPrimitiveHandler(reg, handleForEachPlayer)
	registerPrimitiveHandler(reg, handleDrawForEachExiled)
	registerPrimitiveHandler(reg, handleManifestForEachLinked)
	...
	registerPrimitiveHandler(reg, handleMovePermanent)
	...
	registerPrimitiveHandler(reg, handleKeepOnePerType)
	...
	registerPrimitiveHandler(reg, handleExileTargetSpells)
	registerPrimitiveHandler(reg, handleVentureIntoDungeon)
	registerPrimitiveHandler(reg, handleVentureIntoUndercity)
	registerPrimitiveHandler(reg, handleTakeInitiative)
	registerPrimitiveHandler(reg, handleRevealPutOntoBattlefield)
	registerPrimitiveHandler(reg, handleCastLinkedCardForFree)
	registerPrimitiveHandler(reg, handleRollDiceCreateTokens)
	registerPrimitiveHandler(reg, handleRevealToHandDrainManaValue)
	...
	registerPrimitiveHandler(reg, handleMoveTopOfLibrary)
	...
	registerPrimitiveHandler(reg, handleExilePermanentForPlay)
	registerPrimitiveHandler(reg, handlePlayChosenExiledCard)
	...
	registerPrimitiveHandler(reg, handleCopyCard)
	registerPrimitiveHandler(reg, handlePlayLinkedExiledCard)
	return reg
}
