...
`GameResult` is the structured output from a completed game. It records the winner, elimination order, turn count, and per-turn logs. The `report` package will consume `[]GameResult` to produce deck analytics.
