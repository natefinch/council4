...
func emitSentenceResolvingSyntax(
	...
	var chooseNumberAtRandomCandidates []int
	var controllerChoosesKeepCandidates []int
	for i := range sentences {
		...
		if len(tokens) > 0 && len(sentences[i].Effects) == 0 &&
			...
			case isChooseNumberAtRandomPreludeTokens(tokens):
				chooseNumberAtRandomCandidates = append(chooseNumberAtRandomCandidates, i)
			case isControllerChoosesKeepPreludeTokens(tokens):
				controllerChoosesKeepCandidates = append(controllerChoosesKeepCandidates, i)
	...
	if len(chooseNumberAtRandomCandidates) > 0 && !recognizeTibaltsTrickerySequence(sentences) {
		unrecognizedSibling = true
	}
	if len(controllerChoosesKeepCandidates) > 0 && !recognizeControllerChoosesKeepSequence(sentences) {
		unrecognizedSibling = true
	}
...
func parseSpecialEffects(sentence Sentence, tokens []shared.Token, atoms Atoms) ([]EffectSyntax, bool) {
	for _, recognize := range []func() ([]EffectSyntax, bool){
		...
		func() ([]EffectSyntax, bool) { return parseGrantSpellKeywordEffect(sentence, tokens) },
...
func parseEffects(sentence Sentence, tokens []shared.Token, atoms Atoms) []EffectSyntax {
	...
	for effectIndex, tokenIndex := range indices {
		...
		// A resolving keyword grant to "you and permanents you control" (Dawn's
		// Truce) couples the controller with their permanents under one verb, and
		// its gift-gated sibling "permanents you control also gain ..." separates
		// the group from the verb with an "also" adverb. Both hide the group from
		// the generic static-subject scan (Kind==None); recognize the controlled-
		// permanents group here and flag the leading "you and" coupling so lowering
		// also grants the keyword to the controller as a player-scoped rule effect.
		playerAndControlledPermanents := false
		if kind == EffectGain && staticSubject.Kind == EffectStaticSubjectNone {
			if group, playerAnd, ok := parsePlayerAndControlledPermanentsSubject(subjectTokens, atoms); ok {
				staticSubject = group
				playerAndControlledPermanents = playerAnd
			}
		}
		...
		effects = append(effects, EffectSyntax{
			...
			TokenAttachedToTarget:         tokenAttachedToTarget(kind, clause),
			TokenAttachedToReference:      tokenAttachedToReference(kind, clause),
			...
			PlayerAndControlledPermanents: playerAndControlledPermanents,
			...
			EntersAttacking:               effectWordsAtAny(ownership, "battlefield", "attacking"),
...
func finalizeParsedEffect(effect *EffectSyntax, sentence Sentence, atoms Atoms) {
	...
	if effect.Kind == EffectSearch {
		...
		effect.SearchSameNameAsChosenObject = searchSameNameAsChosenObjectRider(effect)
