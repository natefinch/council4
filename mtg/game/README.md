...
`CardFace` exposes abilities through seven categorized fields: `SpellAbility opt.V[SpellAbilityBody]`, `ActivatedAbilities []ActivatedAbilityBody`, `ManaAbilities []ManaAbilityBody`, `LoyaltyAbilities []LoyaltyAbilityBody`, `TriggeredAbilities []TriggeredAbilityBody`, `ReplacementAbilities []ReplacementAbilityBody`, and `StaticAbilities []StaticAbilityBody`. Card source files populate these fields directly using typed body struct literals and `PlainAbilityContent` or `ModalAbilityContent`.
...
When a card's oracle-text order differs from struct-field order (e.g. static keywords printed before triggered/activated abilities), use an immediately-invoked initializer function and `append` each ability to the appropriate field in oracle order — see `mtg/cards/k/karplusan_forest.go` and CARD-IMPLEMENTATION-GUIDE.md Example 7 for the canonical pattern. When consuming abilities in rules or tests, iterate the categorized fields directly or use `CardFace.AbilityCount()` and `CardFace.BodyAt()` for the stable canonical index order: Spell, Activated, Mana, Loyalty, Triggered, Replacement, Static. `BodyAt()` returns `nil` for replacement-ability slots because replacement definitions are not `AbilityBody` values. Nested abilities granted by `ContinuousEffect.AddAbilities` also use typed `AbilityBody` values.
...
| Replacement | `ReplacementAbilities []ReplacementAbilityBody` | "If … would … instead …" | "If ~ would die, instead …" |
| Static | `StaticAbilities []StaticAbilityBody` | declarative | "Creatures you control get +1/+1." |

50+ keywords are enumerated (flying, haste, deathtouch, lifelink, indestructible, protection, flashback, cascade, discover, eternalize, morph, disguise, etc.). Keyword authoring uses sealed `KeywordAbility` variants on body types that carry keywords (`StaticAbilityBody`, `ActivatedAbilityBody`, and `TriggeredAbilityBody`), such as `SimpleKeyword`, `WardKeyword`, `EnchantKeyword`, `KickerKeyword`, `MorphKeyword`, `DisguiseKeyword`, `SuspendKeyword`, and `ProtectionKeyword`. Plain non-parameterized keywords have reusable `StaticAbilityBody` templates such as `FlyingStaticBody`, `HasteStaticBody`, `ReachStaticBody`, and `ExaltedStaticBody`; append them to `StaticAbilities` rather than mutating their fields. Rules code should use the shared helpers in `keyword.go`, such as `BodyHasKeyword`, `BodyKeywordAbility`, `BodyAddKeywordKindsTo`, `BodyWardCost`, `BodyMadnessCost`, `StaticBodyEnchantTarget`, `ActivatedBodyKicker`, `StaticBodyProtectionColors`, `StaticBodyMorphCost`, `StaticBodyDisguiseCost`, `StaticBodySuspendInfo`, and `EternalizeActivatedBody`.
