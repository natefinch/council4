...
`GenericStrategy` also makes stack-interaction decisions (`stack.go`). Reactive spells — a counter (it targets a spell on the stack) or instant-speed removal (an instant aimed only at permanents) — are scored by their interaction value minus a card-economy cost, with **no base floor**, so a low-value answer scores below `Pass` and the agent holds it for a better target instead of wasting it. A counter is worth more against a costly spell and against one that resolves into a lasting threat (a creature or planeswalker), and the agent never counters its own spell; instant removal is held unless the target's threat is worth a card. This uses the stack characteristics now on the observation (`StackObjectView.ManaValue`/`Types`/`Colors`, redacted for face-down spells).
