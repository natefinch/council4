...
func (v *cardDefValidator) validateFace(faceName, path string, face *CardFace) {
	...
	for i, alternative := range face.AlternativeCosts {
		if alternative.Condition != cost.AlternativeConditionNone &&
			...
			alternative.Condition != cost.AlternativeConditionYourTurn &&
		...
		if alternative.Condition == cost.AlternativeConditionControlsPermanentSubtype &&
			...
			v.add(
				faceName,
				appendPath(path, fmt.Sprintf("AlternativeCosts[%d].ConditionSubtype", i)),
				CardDefIssueInvalidAlternativeCost,
...
func (v *cardDefValidator) validateContinuousEffect(faceName, path string, continuous *ContinuousEffect, targets []TargetSpec) {
	for i := range continuous.AddAbilities {
		...
		if manaAbility, ok := continuous.AddAbilities[i].(*ManaAbility); ok &&
			...
			!IsTapAnyColorCreatureSpellRestrictedManaAbility(manaAbility) {
			v.add(faceName, abilityPath, CardDefIssueInvalidAbilityBody, "continuous effects support only the standard tap-for-one-mana-of-any-color granted mana ability, the bare tap-for-one-mana ability, the Treasure-style sacrifice mana ability, the count-1 sacrifice any-color mana ability, or the any-color mana ability restricted to casting creature spells")
		}
	}
	...
	if continuous.NewControllerRef.Exists {
		if continuous.NewController.Exists {
			v.add(faceName, appendPath(path, "NewControllerRef"), CardDefIssueInvalidReference, "continuous effect sets both NewController and NewControllerRef")
		}
		if continuous.Layer != LayerControl {
			v.add(faceName, appendPath(path, "Layer"), CardDefIssueInvalidAbilityBody, "NewControllerRef requires the control layer")
		}
...
func (v *cardDefValidator) validateTriggerPattern(faceName, path string, pattern *TriggerPattern) {
	...
	if pattern.SelfWasCast && pattern.Event != EventSpellCast {
		v.add(faceName, appendPath(path, "SelfWasCast"), CardDefIssueInvalidSelection, "self-cast trigger filter is only supported for spell-cast events")
	}
...
func (v *cardDefValidator) validateCardRef(faceName, path string, ref CardReference) bool {
	...
	case CardReferenceCaptured:
		if ref.LinkID != "" {
			v.add(faceName, path, CardDefIssueInvalidReference, "captured card reference must not set LinkID")
			return false
		}
		if ref.TargetIndex != 0 {
			v.add(faceName, path, CardDefIssueInvalidReference, "captured card reference must not set TargetIndex")
			return false
		}
