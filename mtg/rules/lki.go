...
func snapshotPermanent(g *game.Game, permanent *game.Permanent, zoneType zone.Type) game.ObjectSnapshot {
	...
	snapshot := game.ObjectSnapshot{
		...
		FaceDownKind: permanent.FaceDownKind,
		MergedCards:  append([]game.MergedCard(nil), permanent.MergedCards...),
		Name:         values.name,
		Owner:        permanent.Owner,
		Controller:   values.controller,
		FromZone:     zoneType,
		Tapped:       permanent.Tapped,
		Attacking:    combatStateMatches(g, permanent, game.CombatStateAttacking),
		Blocking:     combatStateMatches(g, permanent, game.CombatStateBlocking),
		...
		SaddleContributorIDs: append(
			[]id.ID(nil),
			permanent.SaddleContributorIDs...,
		),
