...
func Parse(source string, context Context) (Document, []shared.Diagnostic) {
	...
	source = expandGroupModularKeyword(source)
...
func emitStingCombatFirstStrike(abilities []Ability) {
...
func emitStarCompassMana(abilities []Ability) {
...
func emitUrzasRuinousBlast(abilities []Ability) {
	const text = "Exile all nonland permanents that aren't legendary."
	for i := range abilities {
...
func parseAbility(
	...
	if ability.Kind != AbilityChapter && ability.costPhrase == nil {
		...
		} else if alternative, payment, ok := freeAlternativeCostClause(source, body); ok {
			ability.Kind = AbilitySpellAlternativeCost
			ability.AlternativeCost = alternative
			ability.costPhrase = &payment
		}
	}
	if ability.Kind == AbilityTriggered {
