...
| Type | File | Purpose |
|------|------|---------|
| `CardDef` | `card.go` | Immutable card template — name, mana cost, types, supertypes/subtypes, abilities, P/T, loyalty, battle defense, conditional ETB data, optional hand-written implementation ID, and optional `CardFace` data for double-faced layouts. Shared across games. |
...
| `StackObject` | `stack.go` | A spell or ability on the stack — selected/source face, face-down origin state, source ability index, targets, chosen modes, X value, additional costs. |
...
50+ keywords are enumerated (flying, haste, deathtouch, lifelink, indestructible, protection, flashback, cascade, morph, disguise, etc.). `AbilityDef.ProtectionFromColors` parameterizes Protection for color-based damage prevention and targeting restrictions; `MorphCost` and `DisguiseCost` parameterize face-down turn-up costs. `TargetSpec` supports min/max target counts, legacy natural-language `Constraint` text, broad target categories through `TargetAllow`, and structured `TargetPredicate` data for common color, type, controller, tapped, combat-state, keyword, mana-value, P/T, and "another" filters. `Condition` and `PermanentFilter` model reusable controller-controls predicates for static ability conditions, activation restrictions, trigger intervening-if checks, effect conditions, and replacement effects. Static abilities may carry `Condition` plus `EffectApplyContinuous` templates or `EffectModifyPT` effects with selectors such as `EffectSelectorCreaturesYouControl` or `EffectSelectorOtherCreaturesYouControl`; `mtg/rules` derives those continuous effects dynamically from battlefield and supported non-battlefield zones. `AbilityDef.KickerCost` and `KickerEffects` model the initial Kicker slice. Modal abilities use `Modes` plus `MinModes`, `MaxModes`, and `AllowDuplicateModes` for choose-one, choose-N, up-to-N, one-or-both, all-mode, and duplicate-mode templates. `Effect` stores simple effect primitive data such as `Type`, `Amount`, `DynamicAmount`, `TargetIndex`, `Optional`, `ResultCondition`, `Condition`, `CardCondition`, `Selector`, `PowerDelta`, `ToughnessDelta`, `CounterKind`, `CounterSource`, `ManaColor`, resolution `Choice` / `ChoiceLinkID`, resolution `Payment`, runtime `Replacement`, runtime/static `RuleEffects`, supported library-to-hand `Search`, `Duration`, `UntilEndOfTurn`, `Token`, `ContinuousEffects`, `DelayedTrigger`, `EmblemAbilities`, `LinkID`, and `Description`; the rules engine owns execution behavior. `EffectApplyContinuous` bridges declarative card text to runtime `ContinuousEffect` layer entries for type, subtype, color, ability, control, and P/T-setting changes. `EffectApplyRule` bridges card text to rule-changing effects such as life-gain prohibitions, attack/block prohibitions, generic spell cost modifiers, and graveyard cast permissions. `EffectMoveCounters` moves all counters from a target or triggering event permanent, using last-known information when that permanent has left the battlefield. `EffectChoose` and `EffectPay` model value-producing resolution choices and optional resolution-time payments. `EffectReplace` creates runtime replacement effects for zone-destination changes and simple ETB tapped/counter modifiers. `EffectCounter`, `EffectDiscard`, `EffectSearch`, `EffectReveal`, `EffectShufflePermanentIntoLibrary`, `EffectPutOnBattlefield`, `EffectInvestigate`, `EffectProliferate`, and `EffectGoad` cover reusable keyword-action primitives, including linked reveal-then-act flows. `DynamicAmount` represents effect amounts determined on resolution, such as X, target characteristics, selector counts, counter counts, and linked "that much" values. Linked `EffectResolutionResult` data supports success-aware "if you do" / "if you don't" branches during a single stack-object resolution. `AbilityDef.AdditionalCosts`, `AlternativeCosts`, and runtime `CostModifier` data are used by the rules payment planner, while the legacy `AdditionalCost` string remains only as a compatibility bridge. `CardDef.ImplementationID` is pure data that lets `mtg/rules` route spell resolution to a registered hand-written card implementation when effect primitives are not expressive enough. Current combat rules also consult supported keyword counters such as Flying, First Strike, Deathtouch, Lifelink, Trample, Vigilance, and Indestructible.
