...
During this phase Selection is additive: `TargetSpec.Selection`, `Condition.ControlsMatching`, `Condition.ObjectMatches`, and `TriggerPattern.SubjectSelection`/`CardSelection` are new optional fields, and `TargetPredicate.Selection()`/`PermanentFilter.Selection()` adapt the legacy data to a `Selection` (sharing backing slices, so callers must not mutate the result). `ValidateCardDef` rejects specifying both a legacy filter and its Selection equivalent on the same spec. The single matcher that interprets every `Selection` lives in `mtg/rules`.
