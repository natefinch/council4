...
├── condition.go               # Reusable Condition predicates
...
└── counter/                   # Leaf package: counter types
    ├── README.md              #   Package guide
    └── counter.go             #   Kind enum (27 counter types), Set with +1/+1 ↔ -1/-1 cancel
```
...
`ResolutionPayment.ManaCostMultiplier` evaluates a typed `DynamicAmount` at
resolution and repeats the fixed `ManaCost` exactly; cumulative upkeep uses the
source permanent's resulting age-counter count.
...
Static abilities do not resolve, so they do not use Instructions for continuous or rule-changing behavior. `StaticAbility.ContinuousEffects` and `StaticAbility.RuleEffects` declare that data directly; `mtg/rules` derives the active runtime effects while the static ability functions. A continuous-effect template can set `AffectedSource` to bind its affected object to the source permanent at runtime; it must not also set a `Group`. The rules engine executes typed primitives directly. The `Destroy` primitive carries an optional `PreventRegeneration` flag for "can't be regenerated" destruction, which bypasses regeneration shields while still honoring indestructibility and shield counters.
...
50+ keywords are enumerated (flying, haste, deathtouch, lifelink, indestructible, protection, flashback, cascade, discover, eternalize, embalm, morph, disguise, mutate, read ahead, cumulative upkeep, etc.). Keyword authoring uses sealed `KeywordAbility` variants on ability types that carry keywords (`StaticAbility`, `ActivatedAbility`, and `TriggeredAbility`), such as `SimpleKeyword`, `WardKeyword`, `CumulativeUpkeepKeyword`, `EnchantKeyword`, `KickerKeyword`, `MorphKeyword`, `DisguiseKeyword`, `MutateKeyword`, `ToxicKeyword`, `SuspendKeyword`, and `ProtectionKeyword`. Plain non-parameterized keywords have reusable `StaticAbility` templates such as `DevoidStaticBody`, `FlyingStaticBody`, `HasteStaticBody`, `ReachStaticBody`, `HorsemanshipStaticBody`, `ExaltedStaticBody`, and `ReadAheadStaticBody`; append them to `StaticAbilities` rather than mutating their fields. A Devoid card face must also have an empty `Colors` slice so its colorless characteristic is represented in every zone; its `ColorIdentity` remains unchanged. Complete invariant mechanics use the canonical templates in `mechanic_template.go`: `WardStaticAbility`, `CumulativeUpkeepTriggeredAbility`, `EnchantStaticAbility`, `ProtectionFromColorsStaticAbility`, `CyclingActivatedAbility`, `NinjutsuActivatedAbility`, `MutateStaticAbility`, and `EquipActivatedAbility` coordinate their canonical text, costs, zones, targets, keyword metadata, and instructions; cumulative upkeep adds one age counter, offers the exact fixed mana cost multiplied by the resulting count, and sacrifices through ordinary primitives when unpaid. `TapManaAbility` and `TapManaChoiceAbility` coordinate tap costs, canonical text, resolution choices, and mana production; `TwoColorFilterManaAbility` coordinates the filter-land cycle's hybrid-cost two-color ability (`{X/Y}, {T}: Add {X}{X}, {X}{Y}, or {Y}{Y}.`), adding two mana each chosen from the pair's two colors; `TapManaLandsProduceAbility` coordinates the dynamic "{T}: Add one mana of any color/type that a land you control / an opponent controls could produce." ability (Reflecting Pool, Exotic Orchard, Fellwar Stone), recomputing the choosable colors (and colorless, for the "any type" wording) from the matching battlefield lands' production at resolution; `CantBlockStaticBody`, `CantBeBlockedStaticBody`, `MustAttackStaticBody`, and `CantBeCounteredStaticBody` coordinate exact source-scoped rule declarations with their battlefield and stack zones. Prefer these templates over reconstructing their component fields. Rules code should use the shared helpers in `keyword.go`, such as `BodyHasKeyword`, `BodyKeywordAbility`, `BodyAddKeywordKindsTo`, `BodyWardCost`, `StaticBodyWardCost`, `ActivatedBodyCyclingCost`, `ActivatedBodyEquipCost`, `BodyMadnessCost`, `BodyToxicAmount`, `StaticBodyEnchantTarget`, `StaticBodyMutateCost`, `ActivatedBodyKicker`, `StaticBodyProtectionColors`, `StaticBodyMorphCost`, `StaticBodyDisguiseCost`, `StaticBodySuspendInfo`, `EternalizeActivatedBody`, and `EmbalmActivatedBody`.

`Permanent.MergedCards` records the lower card or token components of a Mutate
permanent in top-to-bottom order. `CardInstanceID`, `Face`, and `TokenDef`
identify the top component, whose characteristics apply to the permanent; the
rules engine aggregates abilities from every component while preserving the
single permanent's object identity and battlefield state.

`Permanent.SagaEntryChapter` records the chapter selected by Read ahead. Zero identifies an ordinary Saga entry; a positive chapter permanently suppresses lower chapter numbers if lore counters are later removed and added again.

`TargetSpec` supports min/max target counts, legacy natural-language `Constraint` text, broad target categories through `TargetAllow`, announcement-time `Chooser` values, and structured `TargetPredicate` data for common color, type, controller, tapped, combat-state, keyword, mana-value, P/T, and "another" filters. Typed counter-placement primitives require their referenced target specification to select only the primitive's matching permanent or player domain; validation infers that domain from legacy constraints when `TargetAllow` is unspecified. For opponent-chosen target slots, predicates such as `ControllerYou` are evaluated relative to the choosing opponent, so `Chooser: TargetChooserOpponent` plus `Controller: ControllerYou` means "a target controlled by that opponent." `Condition` models reusable controller-controls and referenced-object predicates for static ability conditions, activation restrictions, trigger intervening-if checks, instruction conditions, and replacement effects; `Condition.ControlsMatching` applies a `SelectionCount` (a `Selection` plus count and total-power thresholds) to the context controller's permanents. `Condition.ObjectMatches` applies a shared `Selection` to `Condition.Object`; event-permanent references may use last-known information. Conditions also cover life thresholds, alive-opponent counts, individual or collective opponent permanent counts, aggregate controlled-creature total power, exact controller hand size, opponent poison-counter thresholds, class level gates, monstrous-state checks, max-speed checks, and event-permanent name uniqueness for cards such as Guardian Project. `EventHistoryCondition` (stored in `Condition.EventHistory`) checks whether a specific turn's event log — current (`EventHistoryCurrentTurn`) or previous (`EventHistoryPreviousTurn`) — contains at least one event matching a `TriggerPattern`; `Condition.Negate` inverts this for "no events" forms such as "if no spells were cast last turn". `ValidateCardDef` rejects `EventHistoryCondition` values whose `Pattern.Event` is `EventUnknown`.

Modal abilities use `Modes` plus `MinModes`, `MaxModes`, and `AllowDuplicateModes` for choose-one, choose-N, up-to-N, one-or-both, all-mode, and duplicate-mode templates. Exactly one mode with `MinModes: 1` and `MaxModes: 1` is non-modal and executes without recording a choice; `Mode.Ability()` constructs that form. `SearchSpec` supports library-to-hand, library-to-battlefield, and singular shuffle-then-library-top searches with card-type, card-type-any, permanent, supertype, subtype-any, max-mana-value, reveal, shuffle, and enters-tapped options. `FailToFindPolicy` distinguishes qualified or "up to" searches that may return no card from unrestricted exact-card searches that must find one whenever the library is nonempty. An optional `SplitDestination` slot makes an "up to two" search distribute the found cards across two distinct single-card slots — one onto the battlefield (optionally tapped), the other into hand — so the searching player assigns one card per slot, or chooses which slot a lone found card fills (Cultivate, Kodama's Reach). An optional `SharedSubtype` flag makes an "up to two" basic-land search require its found cards to share at least one land subtype, so the runtime only ever finds a legal correlated pair (Myriad Landscape). `DynamicAmount` represents quantities determined on resolution, such as X, target characteristics, object power, selector counts, counter counts, excess damage, and previous instruction results. Declarative `cost.Additional` and `cost.Alternative` values live in `mtg/game/cost`. Spell casting costs are authored on `CardFace.AdditionalCosts` and `CardFace.AlternativeCosts`; activated and mana ability costs remain on their categorized ability bodies, and the payment planner reads those categorized fields directly. Runtime selections remain in `game.AdditionalCostSelection`, while the legacy `AdditionalCost` string remains only as a compatibility bridge. `CardDef.ImplementationID` is pure data that lets `mtg/rules` route spell resolution to a registered hand-written Card Implementation when the typed primitives are not expressive enough.

Activated abilities are authored using `ActivatedAbility` (regular), `ManaAbility` (mana), or `LoyaltyAbility` (planeswalker) and carry mana costs, typed additional costs, source-scoped `CostModifiers`, timing restrictions, target specs, X values, a zone of function, and optional `KeywordAbilities` (e.g. `EquipKeyword`). The rules engine uses those fields for tap mana abilities, Equip, Cycling, Ninjutsu and discard-self Channel abilities from hand, source-exiling graveyard abilities, and general activated abilities with supported effects. Hand and graveyard abilities use the same `StackActivatedAbility` shape with the source card preserved after it moves as a cost. `Game.ActivatedAbilitiesThisTurn` tracks once-per-turn activation guards by source object or source card and ability index.

Triggered abilities use `TriggerCondition.Pattern` to match typed `Event` values. The trigger slice supports exact event-kind matching plus filters for controller, source/self or attached permanent, `ExcludeSelf` for "another" event-source wording, affected player, event-subject/card Selection, zone transition, excluded destination, face-down state, and beginning-of-step events with an explicit `Step`. Combat patterns additionally carry typed Selections for damage sources, damage recipients, attack recipients, and the related blocking combatant; exact attack-recipient kinds and combat/noncombat damage filters; source-as-damage-recipient and attached-permanent-controller step relations; attacker-count relations (`AttackAlone` for "attacks alone" and `AttackerCountAtLeast` for "attack with N or more creatures", both read from `CombatState.Attackers`); and simultaneous or per-attack-target one-or-more batching. `EventAbilityActivated` patterns must set `ExcludeManaAbility` because payment-time mana activations do not yet emit the event; validation and matching reject unrestricted patterns. `EventZoneChanged` patterns can independently constrain the origin and destination; `ExcludeToZone` expresses exact variants such as leaving the battlefield without dying. `TriggerCondition.InterveningCondition` is the structured form of an intervening-if predicate and is checked both when the event triggers and when the ability resolves. Dedicated event predicates cover whether an entering permanent was kicked or was cast and whether an event permanent's current/LKI state had counters or lacked a particular counter kind. `TriggerCondition.State` models simple latched state triggers. Optional "you may" triggered abilities set `TriggeredAbility.Optional`; they still use the stack, and the rules engine asks for the yes/no choice when they resolve. The legacy `TriggerCondition.Event` string is documentation only and is not used for rules behavior.

### Selection

`Selection` (`selection.go`) is pure, valence-agnostic rules data describing **what** characteristics an object must share, never where candidate objects come from. It is the single matcher description that subsumes the characteristic fields previously duplicated across `TargetPredicate`, the permanent/card filters of `TriggerPattern`, and the `EffectSelector` mass-effect constants. The zero value is a wildcard that matches anything; `Empty()` reports the wildcard state and `Validate()` reports structural contradictions (a card type both required and excluded, a supertype both required and excluded, every any-of type/color excluded, a keyword both required and excluded) for `ValidateCardDef`.

Selection separates conjunctive and disjunctive list semantics on purpose, because the legacy types disagreed: `RequiredTypes` and `Supertypes` are all-of (an "artifact creature" type line), while `RequiredTypesAny`, `SubtypesAny`, and `ColorsAny` are any-of ("creature or artifact"). `ExcludedTypes`, `ExcludedColors`, and `ExcludedKeyword` reject when any listed value is present, and the scalar `ExcludedSupertype` rejects a single absent supertype ("nonbasic"/"nonlegendary"/"nonsnow"); one scalar suffices because no canonical Oracle card excludes more than one supertype. Numeric `ManaValue`, `Power`, and `Toughness` use `compare.Int`; `Tapped`, `CombatState`, and keyword fields constrain runtime state. `Controller`/`Player` are relative to a viewing player resolved by the rules adapter, `ExcludeSource` drops the predicate's own source object, `NonToken` rejects tokens, and `TokenOnly` requires them. Counting, total power, and candidate-domain concerns (controlled, defending, equipped, all permanents) stay **outside** Selection and remain with the legacy types until the later reference phase owns runtime binding.

During this phase Selection is additive: `TargetSpec.Selection`, `Condition.ControlsMatching`, `Condition.ObjectMatches`, and `TriggerPattern.SubjectSelection`/`CardSelection` are new optional fields, and `TargetPredicate.Selection()` adapts the legacy target data to a `Selection` (sharing backing slices, so callers must not mutate the result). `ValidateCardDef` rejects specifying both a legacy filter and its Selection equivalent on the same spec. The single matcher that interprets every `Selection` lives in `mtg/rules`.
...
`ContinuousEffect`, `EffectDuration`, `RuleEffect`, `ReplacementEffect`, `PreventionShield`, `CostModifier`, `ObjectSnapshot`, and linked object/card reference types are runtime data owned by `mtg/rules` behavior but stored in `game.Game` so cloned games, agents, logs, and later simulation tooling can observe a complete rules state. The data package defines shapes only; ordering, expiry, replacement, rule-changing, and payment behavior remain in `mtg/rules`. `RuleEffectAttackTax` carries a positive `AttackTaxGeneric` amount and an affected-player relation for fixed generic mana costs imposed per creature attacking that player. `CostModifier` additionally carries `PerObjectReduction` and `CountSelection` for source-scoped dynamic reductions ("This spell costs {N} less to cast for each &lt;countable battlefield object&gt;."): a non-zero `PerObjectReduction` requires `CostModifierSpell` and a `CountSelection` describing the battlefield permanents to count; `validateCostModifier` enforces this pairing. An optional `CountZone` instead scopes that count to the caster's own graveyard or hand cards matching `CountSelection` ("This spell costs {N} less to cast for each &lt;card&gt; in your graveyard."). `CostModifier` also carries an optional `SourceZone` that scopes a spell cost modifier to spells cast from a single zone ("Spells you cast from your graveyard cost {N} less to cast.", Gravebreaker Lamia, Patrician Geist): when present the modifier applies only to spells cast from that zone, and `validateCostModifier` requires it to name a real zone on a `CostModifierSpell`.

### Game Events

`Event` (`event.go`) is the shared typed vocabulary for rules-relevant facts such as spell casts/resolutions, permanents entering or dying, damage dealt or prevented, destruction replacement, cards drawn/discarded/revealed, zone changes, face-up turns, combat declarations, and fights. Supported draw, life-change, scry, and surveil events preserve the affected player's exact event ordinal during the current turn. Permanent zone-change events preserve permanent/card identity, controller, owner, origin, destination, face-down state, and a simultaneous-event ID. Attack events preserve the attacked player and player/planeswalker/battle target; block and fight events preserve both combatants; declaration, fight-resolution, and combat-damage events preserve exact simultaneous batch identity. Permanent-damage events preserve the damaged permanent's controller in `Player`. Permanent-enter events additionally preserve whether the resolved spell was kicked and whether entry came from a cast, non-copy spell. Token events may carry `TokenDef` as last-known definition data because tokens have no `CardInstanceID`. `ObjectSnapshot` preserves departed permanents' effective characteristics, tapped and combat state, counters, attachments, and other runtime state for exact LKI matching and references.

`EventSpellCast` carries `CardTypes` (the types of the face actually cast) and `Colors` (the colors of that face) so triggered-ability pattern matching can filter by spell type and color. The `Colors` field reflects the effective/chosen face at cast time, paralleling `CardTypes`. `CardTypes` and `Colors` are populated by every `EventSpellCast` emission site in `mtg/rules` using the face-specific `cardTypes` and `spellColors` helpers in `faces.go`; face-down casts explicitly record the face-down spell's creature type and no colors.
...
The `counter` package provides 30+ counter kinds (+1/+1, -1/-1, loyalty, charge, time, age, shield, stun, keyword counters, poison, energy, experience, etc.) and a `Set` type that tracks permanent counts per kind. Player poison, energy, and experience totals remain named `Player` fields. Shield counters are consumed by the rules replacement/prevention slice to prevent damage or replace destruction. Includes `CancelOpposites()` for the +1/+1 vs -1/-1 state-based action (CR 704.5r).
...
- **Continuous-effect recipients** — reports `CardDefIssueInvalidReference` when a continuous effect sets both `AffectedSource` and a `Group`.
