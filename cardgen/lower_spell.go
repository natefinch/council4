...
func lowerDealDamageSpell(cardName string, ctx contentCtx) (game.AbilityContent, *shared.Diagnostic) {
	...
	if content, ok := lowerKickerScaledEachTargetDamageSpell(ctx); ok {
		return content, nil
	}
...
func lowerImmediateSingleEffectSpell(
	...
	case compiler.EffectCantBeSacrificed:
		return lowerCantBeSacrificedSpell(ctx)
	...
	case compiler.EffectIncubate:
		return lowerIncubateContent(ctx, syntax)
	...
	case compiler.EffectShuffle:
		...
		if content, ok := lowerEachPlayerGraveyardShuffleIntoLibrary(ctx); ok {
			return content, nil
		}
