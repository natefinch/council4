...
`CardFace` stores categorized abilities: optional `SpellAbility`, lists of `ActivatedAbilities`, `ManaAbilities`, `LoyaltyAbilities`, `TriggeredAbilities`, `ReplacementAbilities`, and `StaticAbilities`, plus legacy `Abilities` while cards migrate. `AbilityDef` (`ability.go`) remains the compatibility view consumed by existing rules paths; `CardFace.AbilityDefs` combines both shapes and returns body-backed `AbilityDef` values even for legacy literals. New definitions should use the categorized fields with sealed ability bodies (`SpellAbilityBody`, `ActivatedAbilityBody`, `ManaAbilityBody`, `LoyaltyAbilityBody`, `TriggeredAbilityBody`, and `StaticAbilityBody`) plus `PlainAbilityContent` or `ModalAbilityContent` for instructions. The legacy `Kind`, `Effects`, `Targets`, `Modes`, and activation/trigger/static fields remain during migration; use the `AbilityDef` accessors such as `IsSpell`, `IsActivated`, `IsMana`, `IsTriggered`, and `StaticBody` when consuming abilities so both shapes work.
