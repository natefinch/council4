...
func lowerExecutableAbility(
	...
	case compiler.AbilitySpell:
		...
		if body.ExactSequence == compiler.ExactSequenceDrawThenDiscardUnlessType {
			spellAbility := lowerDrawThenDiscardUnlessSequence(body)
			return abilityLowering{
				spellAbility: opt.Val(spellAbility),
				sourceSpans:  []shared.Span{body.Content.Span},
			}, nil
		}
		if body.ExactSequence == compiler.ExactSequenceBargainSearchCastPayoff {
			spellAbility := lowerBargainSearchCastPayoffSequence(body)
			return abilityLowering{
				spellAbility: opt.Val(spellAbility),
				sourceSpans:  []shared.Span{body.Content.Span},
			}, nil
		...
		for i := range ability.Content.Effects {
			...
			if ability.Content.Effects[i].TokenCopyHalvePTRoundUp {
				spans = append(spans, ability.Content.Effects[i].TokenCopyHalveRoundUpRiderSpan)
			}
