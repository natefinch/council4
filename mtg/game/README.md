...
`CardFace` exposes abilities through seven categorized fields: `SpellAbility opt.V[SpellAbilityBody]`, `ActivatedAbilities []ActivatedAbilityBody`, `ManaAbilities []ManaAbilityBody`, `LoyaltyAbilities []LoyaltyAbilityBody`, `TriggeredAbilities []TriggeredAbilityBody`, `ReplacementAbilities []ReplacementAbilityDef`, and `StaticAbilities []StaticAbilityBody`. Card source files populate these fields directly using typed body struct literals and `PlainAbilityContent` or `ModalAbilityContent` for instructions. When a card's oracle-text order differs from struct-field order (e.g. static keywords printed before triggered/activated abilities), use an immediately-invoked initializer function and `append` each ability to the appropriate field in oracle order — see `mtg/cards/k/karplusan_forest.go` and CARD-IMPLEMENTATION-GUIDE.md Example 7 for the canonical pattern. `AbilityDef` (`ability.go`) is the compatibility view consumed by existing rules paths; `CardFace.AbilityDefs` returns body-backed `AbilityDef` values with both Body and flat compatibility fields populated. Use the `AbilityDef` accessors `IsSpell`, `IsActivated`, `IsMana`, `IsTriggered`, `IsStatic`, and `FunctionZone` when consuming abilities. The flat `Kind`, `IsManaAbility`, and `IsLoyaltyAbility` fields are set automatically during normalization and should not be written in card source literals. The legacy `Abilities []AbilityDef` field on `CardFace` remains for rules and test compatibility but is not used in registered card source definitions. Nested granted abilities in `ContinuousEffect.AddAbilities` and similar effect data use `AbilityDef` directly because they are not `CardFace` fields.
