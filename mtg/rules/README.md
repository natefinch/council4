...
A mana ability may also carry a mana-spend rider — Path of Ancestry's commander-identity mana whose produced unit scries 1 when spent to cast a creature spell sharing a creature type with the commander. When such a mana ability resolves, `handleAddMana` registers one `game.ManaRiderInstance` per produced unit on the controller's `Player.ManaRiders` (tagging the mana's color and producing permanent). Because all mana leaves the pool through one spend choke point but firing the rider needs spell context, the engine snapshots the casting player's colored pool immediately before paying a spell's costs and the payment planner reports the exact per-color pool mana the payment consumed (`coloredPoolSpend`, threaded out of `paySpellCosts`). Since the planner always spends existing pool mana before tapping new sources, the pre-existing pool mana consumed — the only mana that can carry tags — is `min(before[color], poolSpent[color])`; deriving it from the planner's reported spend rather than a gross before/after pool delta keeps it exact even when a source over-produces the same color mid-payment and leaves a leftover. Equal-color mana is fungible (CR 106.6, 106.12), so on a payment that satisfies a rider's condition the engine spends tagged units first and fires each consumed rider's scry as a `StackTriggeredAbility` sourced from the producing permanent; on any other payment it preserves tagged units (spending plain mana first) and only forced tagged consumption removes a rider without firing. Before each spell snapshot the engine reconciles riders against the current pool, dropping any whose tagged mana was already spent on a non-spell cost (an ability or ward), so those stale riders never fire on a later spell. The rider qualification fails closed when the caster has no single modeled commander, so partner and Background commanders never spuriously scry. The whole path is a no-op for players holding no riders, so ordinary mana carries no overhead.
...
Cost modifiers run after normal/alternative/kicker cost selection and before additional/mana payment planning. Current modifiers cover generic increases, reductions, set values, and minimum generic rules for spells, plus Ghostly Prison-style attack taxes. They also cover source-scoped per-object reductions ("This spell costs {N} less to cast for each &lt;countable battlefield object&gt;.", as on Blasphemous Act): the casting card's own `CostModifier` carries `PerObjectReduction` and a `CountSelection`, the count is computed from the current battlefield with the existing `Selection` machinery at cost time, and the reduction applies only while that exact source spell is cast (it is read directly off the card being cast, never registered as a global rule effect). Generic reductions floor at zero and never touch colored requirements. Ability-specific modifiers and X-value enumeration after reductions remain carry-forward work.
